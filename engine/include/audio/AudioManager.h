...
private:
    struct SoundData
    {
        ...
        SDL_AudioStream* audioStream;// Stream bound to device
        SDL_AudioSpec audioSpec;     // Format/specification for audio
        bool isLooping;              // Whether sound should loop
    };
    
    static SDL_AudioDeviceID audioDevice;               // Active audio device
    static std::map<std::string, SoundData> sounds;     // Sound cache by name
